...
export default {
  ...
  methods: {
    ...
    async modificar() {
      try {
        const { nombre, descripcion } = this.proyecto;
        const res = await fetch(`${API_BASE_URL}/proyectos/${this.proyecto.id}`, {
          method: "PUT",
        ...
        if (!res.ok) {
          throw new Error(`Error al actualizar proyecto: ${res.status}`);
        }
